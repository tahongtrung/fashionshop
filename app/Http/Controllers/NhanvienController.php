...
class NhanvienController extends Controller
{
    ...
    {
        $user = DB::table('users')->where('id',$id)->update(['loainguoidung_id'=>2]);
        return redirect()->route('admin.nhanvien.list')->with(['flash_level'=>'success','flash_message'=>'chuyển nhanvien sang user thành công!!!']);
    }
    public function getList()
    {
        $data = DB::table('users')
            ->where('loainguoidung_id', 3)
            ->get();
            // print_r($data);
